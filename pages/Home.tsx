    ...
    <div className="max-w-xl mx-auto min-h-screen bg-[#F9FAFB] pb-44">
      ...
      <main className="px-6 pt-8 space-y-12">
        {!hasDrafts ? (
          ...
          <>
            <section>
              <div className="mb-4 px-1"><h1 className="text-xl font-black text-gray-900 tracking-tight">발행 예정 요약</h1></div>
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-3">
                    ...
                    <Card onClick={() => navigate(`/draft/${draft.id}`)} className="!p-5 hover:border-blue-200 transition-all border-gray-100 shadow-sm pr-16">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-black text-gray-900 text-[15px] truncate max-w-[100px]">{draft.buyer.name || '상호 미입력'}</h3>
                            <p className="text-[11px] font-bold text-gray-300">{draft.issueDate?.replace(/-/g, '.')}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <p className="font-black text-gray-900 text-[16px]">{formatCurrency(draft.totalAmount)}</p>
                ...
                {drafts.length > 5 && (
                  <button onClick={() => navigate('/history')} className="w-full py-4 text-[13px] font-black text-gray-400 hover:text-blue-500 transition-colors">전체 내역 보기</button>
                )}
              </div>
            </section>
          </>
          ...
          <div className="w-full max-w-xl bg-white rounded-[40px] p-8 animate-in slide-in-from-bottom duration-300 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center mb-6 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-gray-900">{exportStep === 'period' ? '발급 기간 선택' : '발급 항목 확인'}</h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              {exportStep === 'period' ? (
                <div className="space-y-4 pb-4">
                  <ExportOption title="이번 달 자료" subtitle="현재 월 작성분" count={getFilteredDrafts('thisMonth').length} onClick={() => openSelectionStep('thisMonth')} icon={<Calendar size={20} className="text-blue-500" />} highlight />
                  <ExportOption title="지난 달 자료" subtitle="전월 작성분" count={getFilteredDrafts('lastMonth').length} onClick={() => openSelectionStep('lastMonth')} icon={<Calendar size={20} className="text-gray-400" />} />
                  <ExportOption title="전체 자료" subtitle="모든 내역" count={drafts.length} onClick={() => openSelectionStep('all')} icon={<FileDown size={20} className="text-gray-400" />} />
                  <div className="pt-6 border-t border-gray-100">
                    <p className="text-[12px] font-black text-gray-400 mb-3 ml-2 uppercase tracking-tight">직접 기간 설정</p>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold text-gray-400 ml-2">시작일</span>
                        <input type="date" value={customRange.start} onChange={(e) => setCustomRange(prev => ({ ...prev, start: e.target.value }))} className="w-full bg-gray-50 p-4 rounded-2xl text-[14px] font-bold outline-none border-2 border-transparent focus:border-blue-500" />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold text-gray-400 ml-2">종료일</span>
                        <input type="date" value={customRange.end} onChange={(e) => setCustomRange(prev => ({ ...prev, end: e.target.value }))} className="w-full bg-gray-50 p-4 rounded-2xl text-[14px] font-bold outline-none border-2 border-transparent focus:border-blue-500" />
                      </div>
                    </div>
                    <button onClick={() => openSelectionStep('custom')} className="w-full py-4 bg-gray-100 rounded-2xl font-black text-gray-900 text-[14px] hover:bg-gray-200 transition-colors">선택한 기간으로 조회하기 ({getFilteredDrafts('custom').length}건)</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-2 pb-6">
                  <p className="text-[12px] font-bold text-gray-400 mb-4 px-2 italic">발급을 원하지 않는 항목은 체크를 해제하세요.</p>
            ...
            <div className="pt-6 shrink-0 flex gap-3">
              ...
              {exportStep === 'selection' && (
                <button onClick={handleFinalExport} className="flex-[2] h-16 bg-blue-600 text-white rounded-2xl font-black text-[15px] active:scale-95 transition-all shadow-lg shadow-blue-500/20">{selectedIds.length}건 엑셀 발급하기</button>
              )}
            </div>
          </div>
        </div>
      ...
      <nav className="fixed bottom-8 left-6 right-6 max-w-lg mx-auto h-20 bg-gray-900 rounded-[32px] shadow-2xl shadow-black/20 flex items-center justify-between px-6 z-50 border border-white/10 backdrop-blur-lg">
        ...
        <button onClick={() => navigate('/history')} className="flex flex-col items-center gap-1 text-gray-500 w-12"><FileSearch size={20} /><span className="text-[9px] font-black">증빙매칭</span></button>
        ...
        <button onClick={() => navigate('/intro')} className="flex flex-col items-center gap-1 text-gray-500 w-12"><BookOpen size={20} /><span className="text-[9px] font-black">소개</span></button>
